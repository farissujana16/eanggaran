...
<body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color') ?>">
	...
	<script type='text/javascript'>
		var buTable = $('#buTable').DataTable({
			...
			"serverSide": true,
			ajax: {
				url: "<?= base_url() ?>periode/ax_data_periode/",
				type: 'POST'
			},
			columns: [{
					data: "id_permohonan",
					render: function(data, type, full, meta) {
						...
						str += '<li><a href ="<?php echo base_url() ?>periode/details/' + data + '"><i class="fa fa-list"></i> Details</a></li>';
		...
		$('#btnSave').on('click', function() {
			if ($('#bulan').val() == '') {
				...
				var url = '<?= base_url() ?>periode/ax_set_data';
				var data = {
		...
		function ViewData(id_permohonan) {
			if (id_permohonan == 0) {
				$('#addModalLabel').html('Add Periode');
				$('#id_permohonan').val('');
				...
				var url = '<?= base_url() ?>periode/ax_get_data_by_id';
				var data = {
		...
		function DeleteData(id_permohonan) {
			alertify.confirm(
				...
				function() {
					var url = '<?= base_url() ?>periode/ax_unset_data';
					var data = {
						id_permohonan: id_permohonan
					};

...
<body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color') ?>">
						...
						<div class="panel panel-default">
							<div class="panel-heading">
								<button class="btn btn-primary" onclick='ViewData(0)'>
									<i class='fa fa-plus'></i> Add Bidang Access
								</button>
									...
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-body">
												...
												<input type="hidden" id="id_bidang_access" name="id_bidang_access" value='0' />
	...
	<script type='text/javascript'>
		var buTable = $('#buTable').DataTable({
			...
			"serverSide": true,
			ajax: {
				url: "<?= base_url() ?>bidang/ax_data_bidang_access/<?= $id_bidang ?>",
				type: 'POST'
			},
			columns: [{
					data: "id_bidang_access",
		...
		$('#btnSave').on('click', function() {
			if ($('#id_user').val() == 0) {
				...
				var url = '<?= base_url() ?>bidang/ax_set_data_access';
				var data = {
					id_bidang: $('#id_bidang').val(),
					id_bidang_access: $('#id_bidang_access').val(),
					id_user: $('#id_user').val(),
					active: $('#active').val()
				};
		...
		function ViewData(id_bidang_access) {
			if (id_bidang_access == 0) {
				$('#addModalLabel').html('Add Access');
				$('#select2-id_user-container').html('---User--');
				$('#active').val('1');
				$('#addModal').modal('show');
			} else {
				var url = '<?= base_url() ?>bidang/ax_get_data_access_by_id';
				var data = {
					id_bidang_access: id_bidang_access
				};
				...
				$.ajax({
					...
					$('#id_bidang_access').val(data['id_bidang_access']);
		...
		function DeleteData(id_bidang_access) {
			alertify.confirm(
				'Confirmation',
				'Are you sure you want to delete this data?',
				function() {
					var url = '<?= base_url() ?>bidang/ax_unset_data_access';
					var data = {
						id_bidang_access: id_bidang_access
					};
